#German Tax Benefit Calculator clogit 2019

import pandas as pd 

def postgov_by_hours(pid, hours_1, hours_2):
    i = MASEdata_2019.index[MASEdata_2019["pid"]==pid].tolist()[0] #find index of pid
    pid = MASEdata_2019["pid"][i]


    num_of_children= MASEdata_2019["num_of_children"][i]
    n_child_0_1= MASEdata_2019["n_child_0_1"][i]
    n_child_2_4= MASEdata_2019["n_child_2_4"][i]
    n_child_5_7= MASEdata_2019["n_child_5_7"][i]
    n_child_8_10= MASEdata_2019["n_child_8_10"][i]
    n_child_11_12= MASEdata_2019["n_child_11_12"][i]
    n_child_13_15= MASEdata_2019["n_child_13_15"][i]
    n_child_16_18= MASEdata_2019["n_child_16_18"][i]
    private_transfers=MASEdata_2019["private_transfers"][i]
    Alg1= MASEdata_2019["ALG1"][i]
    unemployed_months= MASEdata_2019["unemployed_months"][i]
    employment_status= MASEdata_2019["employment_status"][i]
    annual_wages= MASEdata_2019["annual_wages"][i]
    labor_income_1= float(MASEdata_2019["labor_income"][i])
    if labor_income_1 != labor_income_1:
        labor_income_1 = 0 #!
    age_1= MASEdata_2019["age"][i]
    #worktime_weekly= MASEdata_2019["worktime_weekly"][i]
    Alg2= MASEdata_2019["ALG2"][i]
    partner= MASEdata_2019["partner"][i]
    gender_1= MASEdata_2019["gender"][i]
    maternity_months= MASEdata_2019["maternity_months"][i]
    part_time_experince= MASEdata_2019["part_time_experience"][i]
    full_time_experince= MASEdata_2019["full_time_experience"][i]
    part_time_months= MASEdata_2019["part_time_months"][i]
    hb= MASEdata_2019["hb"][i]
    rent= MASEdata_2019["rent"][i]
    elderly_sa= MASEdata_2019["elderly_sa"][i]
    social_assistance_1= MASEdata_2019["social_assistance"][i]
    if  social_assistance_1 != social_assistance_1:
        social_assistance_1 = 0
    asset_income= MASEdata_2019["asset_income"][i]
    if asset_income != asset_income:
        asset_income = 0
    state= MASEdata_2019["state"][i]
    employment_level= MASEdata_2019["employment_level"][i]
    employment_status_now= MASEdata_2019["employment_status_now"][i]
    post_transfer_income=MASEdata_2019["post_gov_income"][i]
    syear=MASEdata_2019["syear"][i]
    income_last_year_1=MASEdata_2019["income_last_year_1"][i]
    if income_last_year_1 != income_last_year_1:
        income_last_year_1 = 0   
    wage_1=MASEdata_2019["lostwages"][i]/4
    if wage_1 != wage_1:
        wage_1 = 0
    working=MASEdata_2019["working"][i]
    log_wages=MASEdata_2019["log_wages"][i]
    
    
    unemployed_time=MASEdata_2019["unemployed_time"][i]
    partner_in_house=MASEdata_2019["partner_in_house"][i]
    partner_number=MASEdata_2019["partner_number"][i]
    work_experience=MASEdata_2019["work_experience"][i]
    hid=MASEdata_2019["hid"][i]
    disabled_1= MASEdata_2019["disabled_1"][i]
    syear=MASEdata_2019["syear"][i]
    education_1=MASEdata_2019["education"] #şimoş yeni burası
    rhours_1 = MASEdata_2019["hours"][i] #bu line yeni şimoş
    ...
    if partner==1:
        partner_index=MASEdata_2019.index[MASEdata_2019["pid"]==partner_number].tolist()
        if len(partner_index) != 0:
            partner_index = partner_index[0]
            income_last_year_2=MASEdata_2019["income_last_year_1"][partner_index]
            if  income_last_year_2 != income_last_year_2:
                income_last_year_2 = 0                       
            labor_income_2= float(MASEdata_2019["labor_income"][partner_index])
            if labor_income_2 != labor_income_2:
                labor_income_2 = 0  
            wage_2=MASEdata_2019["lostwages"][partner_index]/4
            if wage_2 != wage_2:
                wage_2 = 0
            social_assistance_2= MASEdata_2019["social_assistance"][partner_index]
            if social_assistance_2 != social_assistance_2:
                social_assistance_2 = 0
            disabled_2= MASEdata_2019["disabled_1"][partner_index]
            education_2= MASEdata_2019["education"][partner_index] #bu line yeni şimoş
            age_2= MASEdata_2019["age"][partner_index] #bu line yeni şimoş
            
            rhours_2 = MASEdata_2019["hours"][partner_index] #bu line yeni şimoş
            gender_2 = MASEdata_2019["gender"][partner_index] #bu line yeni şimoş
            if "1" in gender_2:
                gender_2=1
            elif "2" in gender_2:
                    gender_2=2
    ...
    sscp=6700 #monthly assessment ceiling for pension insurance 
    sscu=6700 #monthly assessment ceiling for unemployment insurance
    ...
    if partner==0:
        if wage_1*hours_1==0 and social_assistance_1==0:
            if income_last_year_1>0:
                if num_of_children>0:
                    ALG1_hh = min(0.67*income_last_year_1,6700)*months
                else:
                    ALG1_hh = min(0.60*income_last_year_1,6700)*months
            else:
                ALG1_hh=0
        else:
            ALG1_hh=0
        ...
        if(wage_1*hours_1==0 and social_assistance_1==0)and(wage_2*hours_2>0):
            ...
            if income_last_year_2==0:
                ...
                if num_of_children>0:
                    ALG1_hh=min(0.67*income_last_year_2,6700)*months
                else:
                    ALG1_hh=min(0.60*income_last_year_2, 6700)*months
            ...
            if income_last_year_1+income_last_year_2==0:
                ...
                if num_of_children>0:
                    ALG1_hh=min(0.67*income_last_year_1,6700)*months+min(0.67*income_last_year_2,6700)*months
                else:
                    ALG1_hh=min(0.60*income_last_year_2, 6700)*months+min(0.60*income_last_year_1, 6700)*months
            else:
                ALG1_hh=0
        else:
            ALG1_hh=0           
    ...
    if taxable_income<=9168:
        tax_liability=0
    elif (taxable_income>9168)& (taxable_income<=14254):
        tax_liability= (980.14*((taxable_income-9168)/10000)+1400)*((taxable_income-9168)/10000)
    elif (taxable_income>14254)& (taxable_income<=55960):
        tax_liability= (216.16*((taxable_income-14254)/10000)+2397)*((taxable_income-14254)/10000)+965.58
    elif (taxable_income>55960)& (taxable_income<=265326):
        tax_liability = 0.42 * taxable_income - 8780.90
    else:
        tax_liability = 0.45 * taxable_income - 16740.68
    ...
    if partner==0:
        if (num_of_children==0):
            benefit_amount_hh= 424+404+60.5 
        elif (num_of_children==1):
            benefit_amount_hh= 424+491.4+78.65
        elif (num_of_children==2):
            benefit_amount_hh= 424+604.8+96.8
        elif (num_of_children==3):
            benefit_amount_hh= 424+680.4+108.9
        elif (num_of_children==4):
            benefit_amount_hh= 424+795.6+123.42
        elif (num_of_children>4):
            benefit_amount_hh=424+795.6+93.6*(num_of_children-4)+123.42+14.52*(num_of_children-4)
    elif partner==1:
        if (num_of_children==0):
    ...
    benefit_amount_hh+=245*(n_child_0_1 + n_child_2_4 + 1/3 *n_child_5_7)+ 302*(2/3* n_child_5_7 + n_child_8_10 + n_child_11_12)+322*(n_child_13_15 + 1/3*n_child_16_18)+ 2/3*n_child_16_18*339
...
MASEdata_2019 = pd.read_stata("MASEdata2019.dta")
results2019= []
results2019.append(["partner","syear","hid","pid1","wage1","hours1","chosen_hours1","leisure1","pid2","wage2","hours2","chosen_hours2","leisure2","dpi","gross","choice","getALG1","getother","education_1","education_2","age_1","age_2","state","n_child_0_1", "n_child_2_4","n_child_5_7","n_child_8_10","n_child_11_12","n_child_13_15","n_child_16_18","marital_status","disabled_1","disabled_2"]) #!!!!!!!!!!!!!


seen= set()

for i in range(MASEdata_2019.shape[0]):
    pid = MASEdata_2019["pid"][i]
    if pid in seen:
        continue
    else:
        gender_1= MASEdata_2019["gender"][i] #bu line yeni şimoş
        partner= MASEdata_2019["partner"][i]
        if partner>=1 and partner<=4:
            partner=1
        else:
    ...
    work_experience=MASEdata_2019["work_experience"][i]
    hid=MASEdata_2019["hid"][i]
    education_1= MASEdata_2019["education"][i] #bu line yeni şimoş
    disabled_1= MASEdata_2019["disabled_1"][i]
    ...
    if partner==1:
        ...
        if len(partner_index) != 0:
            ...
            if  labor_income_2 != labor_income_2:
                labor_income_2 = 0
            hours_2=MASEdata_2019["hours"][partner_index]
            if  hours_2 != hours_2:
                hours_2 = 0
            wage_2=MASEdata_2019["lostwages"][partner_index]/4
            if  wage_2 != wage_2:
    ...
    if partner == 1:
        ...
        for job1 in [0,10,20,30,40,50,60]:
            for job2 in [0,10,20,30,40,50,60]:
                ...
                results2019.append([partner,syear,hid,pid,wage_1, job1, chosen_hours1, 80- job1, partner_number, wage_2, job2, chosen_hours2, 80-job2,post_gov_calc,(wage_1 * job1+wage_2*job2)*weeks+asset_income+private_transfers, choice, gets_ALG1, gets_other, education_1, education_2,age_1, age_2, state, n_child_0_1, n_child_2_4,n_child_5_7,n_child_8_10,n_child_11_12,n_child_13_15,n_child_16_18,marital_status,disabled_1,disabled_2]) #results appended bu line yeni şimoş
        chosen_hours1 = min(job_choices, key=lambda x:abs(x-hours_1))
        for job1 in [0,10,20,30,40,50,60]:
            ...
            results2019.append([partner,syear,hid,pid,wage_1, job1, chosen_hours1, 80- job1, 0, 0, 0, 0, 0, post_gov_calc, (wage_1 * job1)*weeks+asset_income+private_transfers,choice, gets_ALG1, gets_other, education_1, "",age_1, "", state, n_child_0_1, n_child_2_4,n_child_5_7,n_child_8_10,n_child_11_12,n_child_13_15,n_child_16_18,marital_status,disabled_1,""])#!!!!!!!!!!!!! results appended bu line yeni şimoş

pdresults= pd.DataFrame(results2019)
pdresults.to_csv("results2019.csv")
