#German Tax Benefit Calculator clogit 2016
...
def postgov_by_hours(pid, hours_1, hours_2):
    i = MASEdata_2016.index[MASEdata_2016["pid"]==pid].tolist()[0] #find index of pid
    pid = MASEdata_2016["pid"][i]


    num_of_children= MASEdata_2016["num_of_children"][i]
    n_child_0_1= MASEdata_2016["n_child_0_1"][i]
    n_child_2_4= MASEdata_2016["n_child_2_4"][i]
    n_child_5_7= MASEdata_2016["n_child_5_7"][i]
    n_child_8_10= MASEdata_2016["n_child_8_10"][i]
    n_child_11_12= MASEdata_2016["n_child_11_12"][i]
    n_child_13_15= MASEdata_2016["n_child_13_15"][i]
    n_child_16_18= MASEdata_2016["n_child_16_18"][i]
    private_transfers=MASEdata_2016["private_transfers"][i]
    Alg1= MASEdata_2016["ALG1"][i]
    unemployed_months= MASEdata_2016["unemployed_months"][i]
    employment_status= MASEdata_2016["employment_status"][i]
    annual_wages= MASEdata_2016["annual_wages"][i]
    ...
    full_time_experince= MASEdata_2016["full_time_experience"][i]
    part_time_months= MASEdata_2016["part_time_months"][i]
    hb= MASEdata_2016["hb"][i]
    rent= MASEdata_2016["rent"][i]
    elderly_sa= MASEdata_2016["elderly_sa"][i]
    social_assistance_1= MASEdata_2016["social_assistance"][i]
    if  social_assistance_1 != social_assistance_1:
        social_assistance_1 = 0
    asset_income= MASEdata_2016["asset_income"][i]
    if asset_income != asset_income:
        asset_income = 0
    state= MASEdata_2016["state"][i]
    employment_level= MASEdata_2016["employment_level"][i]
    employment_status_now= MASEdata_2016["employment_status_now"][i]
    post_transfer_income=MASEdata_2016["post_gov_income"][i]
    syear=MASEdata_2016["syear"][i]
    income_last_year_1=MASEdata_2016["income_last_year_1"][i]
    if income_last_year_1 != income_last_year_1:
        income_last_year_1 = 0   
    wage_1=MASEdata_2016["lostwages"][i]/4
    if wage_1 != wage_1:
        wage_1 = 0
    ...
    if partner==1:
        partner_index=MASEdata_2016.index[MASEdata_2016["pid"]==partner_number].tolist()
        if len(partner_index) != 0:
            partner_index = partner_index[0]
            income_last_year_2=MASEdata_2016["income_last_year_1"][partner_index]
            if  income_last_year_2 != income_last_year_2:
                income_last_year_2 = 0                       
            labor_income_2= float(MASEdata_2016["labor_income"][partner_index])
            if labor_income_2 != labor_income_2:
                labor_income_2 = 0  
            wage_2=MASEdata_2016["lostwages"][partner_index]/4
            if wage_2 != wage_2:
                wage_2 = 0
    ...
    #Child Benefits
    child_benefit1= 190 #benefit amount for the first two children
    child_benefit2= 196 #third children
    child_benefit3= 221 #rest
    ...
    ssch=4237.5 #monthly assessment ceiling for health insurance
    sscl=4237.5 #monthly assessment ceiling for long-term care insurance
    sscp=6200 #monthly assessment ceiling for pension insurance 
    sscu=6200 #monthly assessment ceiling for unemployment insurance
    ...
    if taxable_income<=8652:
        tax_liability=0
    elif (taxable_income>8652)& (taxable_income<=13669):
        tax_liability= (993.62*((taxable_income-8652)/10000)+1400)*((taxable_income-8652)/10000)
    elif (taxable_income>13669)& (taxable_income<=53665):
        tax_liability= (225.40*((taxable_income-13669)/10000)+2397)*((taxable_income-13669)/10000)+952.48
    elif (taxable_income>53665)& (taxable_income<=254446):
        tax_liability = 0.42 * taxable_income - 8394.14
    else:
        tax_liability = 0.45 * taxable_income - 16027.52
    ...
    if partner==0:
        if (num_of_children==0):
            benefit_amount_hh= 404+316+99
        elif (num_of_children==1):
            benefit_amount_hh= 404+453+148
        elif (num_of_children==2):
            benefit_amount_hh= 404+529+176
        elif (num_of_children>=3):
            benefit_amount_hh= 404+625+210
    elif partner==1:
        if (num_of_children==0):
            benefit_amount_hh= 364*2+402+138
        elif (num_of_children==1):
            benefit_amount_hh= 364*2+545+178
        elif (num_of_children==2):
            benefit_amount_hh= 364*2+622+204
        elif (num_of_children>=3):
            benefit_amount_hh= 364*2+738+242
        
            
    benefit_amount_hh+=237*(n_child_0_1 + n_child_2_4 + 1/3 *n_child_5_7)+ 270*(2/3* n_child_5_7 + n_child_8_10 + n_child_11_12)+306*(n_child_13_15 + 1/3*n_child_16_18)+ 2/3*n_child_16_18*324
    ...
    if ALG2_hh>0:
        ...
        if (num_hh_members==1)& (210<post_tax_income<820):
            if rent<= 358:
                hb_hh= 1.08*(rent-(a+b*rent+c*post_tax_income)*post_tax_income)
            else:
                hb_hh=0
        elif (num_hh_members==2):
            if partner==0:
                if 250<post_tax_income<1120:
                    if rent<= 435:
                        hb_hh= 1.08*(rent-(a+b*rent+c*post_tax_income)*post_tax_income)
                    else:
                        hb_hh=0
...
MASEdata_2016 = pd.read_stata("MASEdata2016.dta")
results2016= []
results2016.append(["partner","syear","hid","pid1","wage1","hours1","chosen_hours1","leisure1","pid2","wage2","hours2","chosen_hours2","leisure2","dpi","gross","choice","getALG1","getother","education_1","education_2","age_1","age_2","state","n_child_0_1", "n_child_2_4","n_child_5_7","n_child_8_10","n_child_11_12","n_child_13_15","n_child_16_18","marital_status","disabled_1","disabled_2"]) #!!!!!!!!!!!!!
seen= set()

for i in range(MASEdata_2016.shape[0]):
    pid = MASEdata_2016["pid"][i]
    if pid in seen:
        continue
    else:
    ...
    if partner==1:
        ...
        if len(partner_index) != 0:
            ...
            if  labor_income_2 != labor_income_2:
                labor_income_2 = 0
            hours_2=MASEdata_2016["hours"][partner_index]
            if  hours_2 != hours_2:
                hours_2 = 0
            wage_2=MASEdata_2016["lostwages"][partner_index]/4
            if  wage_2 != wage_2:
...
pdresults.to_csv("results2016.csv")
